...
/opt/redislabs/bin/crdb-cli crdb create --name sample-crdb --memory-size 50mb --default-db-config '{ "port": 12000, "replication": false}' --instance fqdn=${CLUSTER_NAME},username=${ADMIN_U},password=${ADMIN_P} --instance fqdn=${CLUSTER_NAME2},username=${ADMIN_U},password=${ADMIN_P}
EOF
